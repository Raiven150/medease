...
export async function renderSales(filter = "") {
  ...
  // Use event delegation for delete buttons to ensure handlers work reliably
    ...
    if (tableBodyEl) {
      tableBodyEl.addEventListener("click", async (event) => {
        ...
        showConfirm("Delete this sale record?", async () => {
          ...
          // Perform deletion atomically using the provided transaction stores
          await runTransaction(
            [STORES.SALES, STORES.PROFITS, STORES.INVENTORY],
            "readwrite",
            async ({ stores }) => {
              // IMPORTANT: use the transaction's inventory store to get all inventory
              const invReq = stores[STORES.INVENTORY].getAll();
              const inventory = await new Promise((resolve, reject) => {
                invReq.onsuccess = () => resolve(invReq.result);
              ...
              for (const item of sale.items) {
                ...
                const grossMargin =
                  ((item.price ?? 0) - buyingPrice) * item.qty;
