...
export async function renderDashboard() {
  ...
  const todayProfitEl = document.getElementById("dashboard-today-profit");
  if (todayProfitEl) todayProfitEl.textContent = `৳${formatMoney(todayProfit)}`;

  const monthlyProfitEl = document.getElementById("dashboard-monthly-profit");
  if (monthlyProfitEl)
    monthlyProfitEl.textContent = `৳${formatMoney(monthlyProfit)}`;
...
export async function initDashboard() {
  await renderDashboard();
  // Update every second so the time is live
  setInterval(() => {
