  ...
  (await getAll(STORES.SUPPLIERS))
    ...
    .forEach((s) => {
      ...
      row.innerHTML = `
        ...
        <td>
          <button class="secondary-btn edit-supplier"  data-id="${escapeHtml(s.id)}">Edit</button>
          <button class="danger-btn   delete-supplier" data-id="${escapeHtml(s.id)}">Delete</button>
        </td>
      `;
    ...
    btn.onclick = async () => {
      ...
      const html = `
        <div class="form-group">
          ...
          <input type="text" id="edit-company-name" value="${escapeHtml(supplier.companyName)}" />
        </div>
        <div class="form-group">
          <label for="edit-supplier-name">Supplier Name</label>
          <input type="text" id="edit-supplier-name" value="${escapeHtml(supplier.supplierName)}" />
        </div>
        <div class="form-group">
          <label for="edit-contact">Contact</label>
          <input type="text" id="edit-contact" value="${escapeHtml(supplier.contact)}" />
        </div>
      `;
  ...
  document.querySelectorAll(".delete-supplier").forEach((btn) => {
    btn.onclick = async () => {
      ...
      showConfirm("Delete this supplier?", async () => {
        const supplier = await getById(STORES.SUPPLIERS, id);
        if (!supplier) return;
        // Soft delete — record is preserved in DB, filtered from all renders
...
export function initSupplierForm() {
  ...
  form.onsubmit = async (e) => {
    ...
    if (!companyName || !supplierName || !contact) {
      showAlert("Validation Error", "All fields are required.");
      return;
    }

    await addWithoutId(STORES.SUPPLIERS, {
      companyName,
      supplierName,
      contact,
    });
...
export function initSupplierImportExport() {
  // Export — only include non-deleted suppliers so ghost records are not spread
  document.getElementById("export-suppliers").onclick = async () => {
    const data = (await getAll(STORES.SUPPLIERS)).filter((s) => !s.deleted);
  ...
  document.getElementById("import-suppliers-btn").onclick = async () => {
    ...
    // put() upserts by id — prevents duplicates on reimport
